...
data = DataPortal()
data.load(filename = 'opt_model_data/2022_ERCOT_data.csv', select = ('t', 'solar', 'wind', 'demand'),
          param = [model.solar,model.wind,model.demand], index = model.t)
...
def obj_expression(model):
    return sum(model.cap[i] * model.costs[i] for i in model.tech) + sum(model.ESS_d[t] * ESS_p_var_cost + model.curt[t] * curtailment_cost for t in
model.t)
model.OBJ = Objective(rule=obj_expression)

# supply/demand match constraint
def match_const(model, i):
    return model.solar[i]*model.cap['s_cap'] + model.ESS_d[i] - model.ESS_c[i] -model.curt[i] - model.demand[i] == 0
